...
export async function subscribeToNewsletter(
  ...
  // EXPERT PATTERN: Check server-side secret first (Best Practice), then fallback to public (Legacy/Local)
  const webhookUrl =
    process.env.N8N_WEBHOOK || process.env.NEXT_PUBLIC_N8N_NEWSLETTER_WEBHOOK;

  if (!webhookUrl) {
    console.error(
      "CRITICAL ERROR: Helper 'N8N_WEBHOOK' is not defined in environment variables.",
    );
    return {
      success: false,
      message:
